...
func _on_star_size_changed(data) -> void:
	temperature_value.text = str(data[TEMPERATURE_KEY]).pad_decimals(1) + " °K  "
	luminosity_value.text = str(data[LUMINOSITY_KEY]).pad_decimals(4) + " L ⊙  "
	mass_value.text = str(data[MASS_KEY]).pad_decimals(2)  + " M ⊙  "
